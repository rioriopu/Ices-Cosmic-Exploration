...
namespace ICE.Scheduler
{
    internal static unsafe class SchedulerMain
    {
        ...
        /// <summary>
        /// ミッション遂行中にユーザーがゲーム内で採取/製作ジョブを手動で切り替えたら、現ミッションを放棄(または報告)し、
        /// SelectedJob を新ジョブへ追従させて、新ジョブのミッションを受注し直す。
        /// 受注時にプラグインが切り替えるジョブ(Mission_ChangeJob)と区別するため、「現ジョブが現ミッションの対応ジョブに
        /// 含まれない」ことを手動切替の判定とする(受注後は Player.Job がミッションのジョブと一致しているため)。
        /// </summary>
        private static void CheckManualJobSwitch()
        {
            // 作業状態かつミッション受注中のみ対象(GrabMission 中は受注時の一時切替と紛れるため除外)。
            ...
            // Agenda モードはジョブをモード側が管理するので追従対象外
            ...
            // 現ジョブが現ミッションの対応ジョブに含まれる = 正常(受注時の切替)。含まれない = ユーザーの手動切替。
            ...
            // デバウンス(2秒): ジョブチェンジ直後や一時切替での誤検知を避ける
            ...
            IceLogging.Info($"ジョブ手動切替を検知(job={job})。現ミッションを破棄し、SelectedJob を {job} へ追従して新ジョブのミッションを受注し直します", "[JobSwitch]");
            Mission_Settings.SelectedJob = job;
            P.TaskManager.Tasks.Clear();
            if (P.Navmesh.Installed && P.Navmesh.IsRunning())
                P.Navmesh.Stop();
            // ランク未達なら放棄、到達済みなら報告(Task_AbandonMission 内で判定)。完了後は Start に戻り、
            // 更新済みの SelectedJob で新ジョブのミッションが選ばれる。
